...
from core.domain_types import Event
...
class RedisEventBus(IEventBus):
    ...
    def __init__(self):
        ...
        self.pubsub: aioredis.PubSub | None = None
    ...
    async def _handle_message(self, message: dict):
        ...
        try:
            ...
            if channel in self.subscriptions:
                ...
                asyncio.get_event_loop().run_in_executor(self.executor, handler, event)

        except Exception as e:
            logger.error(f"Error handling message: {e}")

    async def publish(
        self, event_type: str, data: dict, metadata: dict | None = None
    ) -> str:
        """Publish an event."""
        try:
            redis = await self._get_redis()
    ...
    async def request(self, channel: str, data: dict, timeout: float = 5.0) -> dict:
        ...
        try:
            ...
            try:
                ...
                raise TimeoutError(
                    f"Request timed out after {timeout} seconds", timeout
                )
