...
@pytest.mark.asyncio
async def test_create_trading_instance(
    trading_client: AsyncClient, create_trading_instance, db_session, test_user
) -> None:
...
@pytest.mark.asyncio
async def test_list_trading_instances(
    trading_client: AsyncClient, create_trading_instance, db_session, test_user
) -> None:
    instance = await create_trading_instance(db_session, user=test_user)
...
@pytest.mark.asyncio
async def test_get_trading_instance(
    trading_client: AsyncClient, create_trading_instance, db_session, test_user
) -> None:
    instance = await create_trading_instance(db_session, user=test_user)
...
@pytest.mark.asyncio
async def test_prepare_and_start_trading_instance(
    trading_client: AsyncClient, create_trading_instance, db_session, test_user
) -> None:
    instance = await create_trading_instance(db_session, user=test_user)
    response = await trading_client.post(
        f"/api/v1/trading-instances/{instance.id}/prepare"
    )
    assert response.status_code == 200
    data = response.json()
    ...
    response = await trading_client.post(
        f"/api/v1/trading-instances/{instance.id}/start"
    )
    assert response.status_code == 200
    data = response.json()
...
@pytest.mark.asyncio
async def test_start_requires_prepare(
    trading_client: AsyncClient, create_trading_instance, db_session, test_user
) -> None:
    instance = await create_trading_instance(db_session, user=test_user)
    response = await trading_client.post(
...
@pytest.mark.asyncio
async def test_pause_resume_stop_trading_instance(
    trading_client: AsyncClient, create_trading_instance, db_session, test_user
) -> None:
    instance = await create_trading_instance(db_session, user=test_user)
    ...
    response = await trading_client.post(
        f"/api/v1/trading-instances/{instance.id}/pause"
    )
    assert response.status_code == 200
    assert response.json()["status"] == "paused"

    response = await trading_client.post(
        f"/api/v1/trading-instances/{instance.id}/resume"
    )
    assert response.status_code == 200
    assert response.json()["status"] == "running"

    response = await trading_client.post(
        f"/api/v1/trading-instances/{instance.id}/stop"
    )
    assert response.status_code == 200
...
@pytest.mark.asyncio
async def test_delete_stopped_instance(
    trading_client: AsyncClient, create_trading_instance, db_session, test_user
) -> None:
    instance = await create_trading_instance(db_session, user=test_user)
    await trading_client.post(f"/api/v1/trading-instances/{instance.id}/prepare")
...
@pytest.mark.asyncio
async def test_cannot_delete_running_instance(
    trading_client: AsyncClient, create_trading_instance, db_session, test_user
) -> None:
    instance = await create_trading_instance(db_session, user=test_user)
    await trading_client.post(f"/api/v1/trading-instances/{instance.id}/prepare")
